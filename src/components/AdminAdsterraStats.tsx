...
/** Stats for this admin’s site only — never other Adsterra websites. */
export default function AdminAdsterraStats() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      // No domain=all — API always scopes to this site’s Adsterra domain.
  ...
  const siteLabel = data?.autoDomainTitle || "this site";

  return (
    <div className="space-y-4">
      <div className="rounded-md border border-ink-700 bg-ink-950 px-3 py-2 text-xs text-ink-400">
        Showing stats for{" "}
        <span className="font-medium text-ink-100">{siteLabel}</span> only
        {data?.siteUrl ? (
          <>
            {" "}
            · site URL <span className="text-ink-300">{data.siteUrl}</span>
          </>
        ) : null}
      </div>
      ...
      {loading && !data ? (
        ...
        <>
          ...
          {!data?.domainId && !data?.autoDomainId ? (
            <p className="rounded-md border border-amber-900/40 bg-ink-950 px-3 py-2 text-xs text-amber-400">
              Could not match this site to an Adsterra website. Open Admin → Adsterra →{" "}
              <strong>Stats website</strong> and pick{" "}
              <code className="text-ink-300">luugyizcar.site</code> or{" "}
              <code className="text-ink-300">akogyivip.site</code>, then Save. Also set{" "}
              <code className="text-ink-300">NEXT_PUBLIC_SITE_URL</code> to that domain.
            </p>
          ) : null}
                  ...
                  items.map((row, i) => (
                    <tr key={i} className="text-ink-300">
                      <td className="px-3 py-2 text-ink-100">
                        {rowLabel(row, groupBy, placementMap)}
                      </td>
